...
import { formatAreaPreview } from '@/lib/format';
...
export function SmartAreaField({ label, value, min, max, step, unit, tooltip, onChange }: Props) {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isNaN(num)) {
      onChange(Math.min(Math.max(num, 0), max)); 
    }
  };
  ...
  const preview = formatAreaPreview(value);
